...
f.close()
print("Number of value errors [Glove tokens that can't be used] %d", num_value_errors)
...
# model.load_weights(filepath="weights.07-0.05.hdf5", by_name=False)

print("Making predictions")
predictions = model.predict(data_test, batch_size=batch_size)
test_ids = test_ids.reshape((len(test_ids), 1))

test_predicts = pd.DataFrame(data=predictions, columns=CLASSES)
